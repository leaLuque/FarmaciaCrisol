...
class FraccionarProducto(CRUDWidget, Ui_vtnFraccionarProducto):
    ...
    def cargarFraccionables(self):
        ...
        self.filtrar(infoItem[3])

    def filtrar(self, lote):
        codigo = str(lote)
        data = self.getAllTabla(self.tablaFraccionable)

        dataProdFrac = filter(lambda x: x[3].upper() == codigo.upper(), data.values())

        for dato in data:
            self.tablaFraccionable.setRowHidden(dato, False)

        for dato in data:
            if not data[dato] in dataProdFrac:
...
class AjusteNegativoStock(CRUDWidget, Ui_vtnAjusteNegativoStock):
    ...
    def buscar(self):
        ...
        if lote != "":
            data_lote = filter(lambda x: x[3].upper() == lote.upper(), data_producto)
        else:
