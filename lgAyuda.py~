...
class Ayuda (MdiWidget, Ui_vtnAyuda):
    ...
    def cargarContenido(self, id):
        ...
        if (id == "id_menu_usuario" or id == "id_ing"):
            ...
            self.leerTxt("Salir", "", "", "salir", "id_menu_usuario")
            ...
            self.leerTxt("Alta Presentación", "img/capturas/altaProd.png", "", "altaPres", "id_menu_producto")
        elif (id == "id_baja_pres"):
            self.leerTxt("Baja Presentación", "img/capturas/bajaProd.png", "", "bajaProd", "id_menu_producto")
        elif (id == "id_mod_pres"):
            self.leerTxt("Modificar Presentación", "img/capturas/modProd.png", "", "modProd", "id_menu_producto")
    ...
    def leerTxt(self, titulo, imagen, figura, pal_clave, id_menu_onclick):
        ...
        body = """<h1 id="titulo"> {titulo} </h1>
            <div class="imgbox">
                ...
                <br/>
                <p> {figura} </p>
            </div>""".format(titulo=titulo, imagen=imagen, figura=figura)
        ...
        for m in re.finditer(patron, contenido_txt):
            ...
            for match in re.split('#parrafo', sub):
                parrafo = """<p><strong> {parrafo} </strong></p>""".format(parrafo=match)
                body += parrafo
        ...
        for id, menu in self.id_menu:
            if (id == id_menu_onclick):
                for m in menu:
                    ind = """<li> <a href="#" id={id} onclick={onclick}>{txt}</a></li>""".\
